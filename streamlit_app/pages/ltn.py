"""
Página LTN - Letra do Tesouro Nacional
"""
...
def render():
    """
    Renderiza a página de cálculo LTN
    """
    st.title("📈 LTN - Letra do Tesouro Nacional")
    st.markdown("---")
    ...
    with col1:
        data_vencimento = st.date_input("Data de Vencimento", value=date(2025, 1, 1))
    
    with col2:
        taxa = st.number_input("Taxa (%)", min_value=0.0, value=12.5, step=0.01, format="%.2f")
    
    with col3:
        dias_liquidacao = st.number_input("Dias para Liquidação", min_value=0, value=1)
    
    col4, col5 = st.columns(2)
    
    with col4:
        quantidade = st.number_input("Quantidade", min_value=0.0, value=0.0, step=1.0)
    
    with col5:
        financeiro = st.number_input("Valor Financeiro (R$)", min_value=0.0, value=0.0, step=1000.0)
    ...
    if st.button("Calcular", type="primary"):
        ...
        try:
            with st.spinner("🔄 Calculando..."):
                response = requests.post(f"{API_URL}/titulos/ltn", json=data_request, timeout=30)
