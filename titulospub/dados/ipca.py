...
def inicio_fim_mes_ipca(data: pd.Timestamp, feriados: Union[list, pd.Series, None]) -> tuple:
...
def dicionario_ipca(data: pd.Timestamp, ipca_fechado_df: pd.DataFrame, ipca_proj_float:float, feriados=None):
    ...
    if not e_dia_util(data=dia_15_mes_atu, feriados=feriados):
            dia_15_mes_atu = adicionar_dias_uteis(data=dia_15_mes_atu, num_dias=1, feriados=feriados)
