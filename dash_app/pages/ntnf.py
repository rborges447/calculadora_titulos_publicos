...
def layout():
    return dbc.Container(
        [
            ...
            html.H2("NTNF - Nota do Tesouro Nacional - Série F", className="mb-3"),
                            ...
                            dbc.Card(
                                dbc.CardBody(
                                    [
                                            ...
                                            [
                                                ...
                                                dbc.Col(
                                                    [
                                                        html.Div(
                                                            [
                                                                dbc.Label(id="ntnf-calc-taxa-label", children="Taxa (%)"),
                                                                dbc.Input(
                                                                    id="ntnf-calc-taxa",
                                                                    type="text",
                                                                    placeholder="Vazio = usa ANBIMA",
                                            ...
                                            [
                                                ...
                                                dbc.Col(
                                                    [
                                                        html.Div(
                                                            [
                                                                dbc.Label(id="ntnf-calc-valor-label", children="Quantidade"),
                                                                dbc.Input(
                                                                    id="ntnf-calc-valor",
                                                                    type="number",
                                                                    min=0,
                                        ...
                                        dbc.Button(
                                            "Calcular",
                                            id="ntnf-calc-calcular",
                                            color="primary",
                                            className="mb-3",
                                        ),
                                        html.Div(id="ntnf-calc-resultado"),
                                    ]
                                ),
                            ),
                        ],
...
@callback(
    [
        Output("ntnf-eq-titulo-a", "options"),
        Output("ntnf-eq-titulo-b", "options"),
        Output("ntnf-calc-vencimento", "options"),
    ],
    [
        Input("ntnf-dados-originais", "data"),
    ],
)
...
@callback(
    Output("ntnf-calc-valor-label", "children"),
    Input("ntnf-calc-modo", "value"),
)
...
@callback(
    [
        Output("ntnf-calc-box-taxa", "style"),
        Output("ntnf-calc-box-premio-di", "style"),
        Output("ntnf-calc-taxa-label", "children"),
    ],
    Input("ntnf-calc-tipo", "value"),
)
...
@callback(
    Output("ntnf-eq-resultado", "children"),
    [Input("ntnf-eq-calcular", "n_clicks"), Input("ntnf-dias", "value")],
    [
        State("ntnf-eq-titulo-a", "value"),
        State("ntnf-eq-titulo-b", "value"),
        State("ntnf-eq-quantidade-a", "value"),
        State("ntnf-eq-taxa-a", "value"),
        State("ntnf-eq-taxa-b", "value"),
        State("ntnf-eq-criterio", "value"),
        State("ntnf-dados-originais", "data"),
    ],
    prevent_initial_call=True,
)
def calcular_equivalencia(n_clicks, dias_liquidacao, titulo_a, titulo_b, qtd_a, taxa_a, taxa_b, criterio, dados_originais):
    ...
    if trigger_id == "ntnf-dias" and (not titulo_a or not titulo_b):
        return html.Div("Preencha os campos e clique em Calcular.", className="text-muted")
    ...
    payload = {"titulo1": "NTNF", "venc1": titulo_a, "titulo2": "NTNF", "venc2": titulo_b, "qtd1": float(qtd_a), "criterio": criterio_api}
...
@callback(
    Output("ntnf-calc-resultado", "children"),
    [
        Input("ntnf-calc-calcular", "n_clicks"),
        Input("ntnf-dias", "value"),
    ],
    [
        State("ntnf-calc-vencimento", "value"),
        State("ntnf-calc-tipo", "value"),
        State("ntnf-calc-taxa", "value"),
        State("ntnf-calc-premio", "value"),
        State("ntnf-calc-di-valor", "value"),
        State("ntnf-calc-modo", "value"),
        State("ntnf-calc-valor", "value"),
        State("ntnf-dados-originais", "data"),
    ],
    prevent_initial_call=True,
)
def calcular_detalhado(n_clicks, dias_liquidacao, vencimento, tipo, taxa, premio, di_valor, modo, valor, dados_originais):
    ...
    if trigger_id == "ntnf-dias" and (not vencimento or not valor):
        return html.Div("Preencha os campos e clique em Calcular.", className="text-muted")
    ...
    ok, resultado = post("/titulos/ntnf", payload, timeout=30)
