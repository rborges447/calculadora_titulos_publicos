...
class LTNRequest(BaseModel):
    ...
    data_vencimento: str = Field(
        ..., description="Data de vencimento do título (formato: YYYY-MM-DD)", example="2025-01-01"
    )
    ...
    dias_liquidacao: Optional[int] = Field(
        1, description="Dias para liquidação (padrão: 1)", ge=0, example=1
    )
    ...
    premio: Optional[float] = Field(
        None, description="Prêmio sobre DI em pontos base (ex: 0.5 = 50 bps)", example=0.5
    )
...
class NTNBRequest(BaseModel):
    ...
    data_vencimento: str = Field(
        ..., description="Data de vencimento do título (formato: YYYY-MM-DD)", example="2035-05-15"
    )
    data_base: Optional[str] = Field(
        None,
        description="Data base para cálculos (formato: YYYY-MM-DD). Se não informado, usa a data atual",
    ...
    premio: Optional[float] = Field(
        None, description="Prêmio sobre DAP em pontos base (ex: 0.5 = 50 bps)", example=0.5
    )
...
class NTNFRequest(BaseModel):
    ...
    premio: Optional[float] = Field(
        None, description="Prêmio sobre DI em pontos base (ex: 0.3 = 30 bps)", example=0.3
    )
    di: Optional[float] = Field(
        None,
        description="Taxa DI de referência (%). Usado junto com premio para calcular taxa",
...
class NTNBHedgeDIRequest(BaseModel):
    ...
    codigo_di: str = Field(..., description="Código do contrato DI (ex: DI1F32)", example="DI1F32")
    data_base: Optional[str] = Field(
        None,
        description="Data base para cálculos (formato: YYYY-MM-DD). Se não informado, usa a data atual",
        example="2024-12-01",
...
class EquivalenciaRequest(BaseModel):
    ...
    titulo1: str = Field(..., description="Tipo do primeiro título", example="LTN")
    venc1: str = Field(
        ...,
        description="Data de vencimento do primeiro título (formato: YYYY-MM-DD)",
        example="2025-01-01",
    )
    titulo2: str = Field(..., description="Tipo do segundo título", example="NTNB")
    venc2: str = Field(
        ...,
        description="Data de vencimento do segundo título (formato: YYYY-MM-DD)",
        example="2035-05-15",
    )
    qtd1: float = Field(..., description="Quantidade do primeiro título", gt=0, example=10000)
    tx1: Optional[float] = Field(
        None, description="Taxa do primeiro título (%). Se não informado, usa ANBIMA", example=12.5
    )
    tx2: Optional[float] = Field(
        None, description="Taxa do segundo título (%). Se não informado, usa ANBIMA", example=7.53
    )
    criterio: str = Field(
        ..., description="Critério de equivalência: 'dv' (DV01) ou 'fin' (financeiro)", example="dv"
    )
...
class NTNBResponse(BaseModel):
    ...
    data_vencimento_duration: Optional[str] = Field(
        None, description="Data de vencimento da duration"
    )
...
class VencimentosResponse(BaseModel):
    ...
    total: int = Field(..., description="Total de vencimentos disponíveis", example=13)
...
class CodigosDIResponse(BaseModel):
    ...
    codigos: list[str] = Field(
        ..., description="Lista de códigos DI disponíveis", example=["DI1F32", "DI1F33", "DI1F34"]
    )
    total: int = Field(..., description="Total de códigos DI disponíveis", example=15)
...
class TodosVencimentosResponse(BaseModel):
    ...
    ltn: list[str] = Field(
        ..., description="Vencimentos disponíveis para LTN", example=["2026-01-01", "2026-04-01"]
    )
    lft: list[str] = Field(
        ..., description="Vencimentos disponíveis para LFT", example=["2026-01-01", "2026-04-01"]
    )
    ntnb: list[str] = Field(
        ..., description="Vencimentos disponíveis para NTNB", example=["2026-01-01", "2026-04-01"]
    )
    ntnf: list[str] = Field(
        ..., description="Vencimentos disponíveis para NTNF", example=["2026-01-01", "2026-04-01"]
    )
...
class CarteiraCreateRequest(BaseModel):
    ...
    quantidade_padrao: Optional[float] = Field(
        None, description="Quantidade padrão para cada título", gt=0, example=50000
    )
...
class CarteiraUpdateTaxaRequest(BaseModel):
    ...
    vencimento: str = Field(
        ..., description="Data de vencimento (formato: YYYY-MM-DD)", example="2025-01-01"
    )
    taxa: float = Field(..., description="Nova taxa de juros (%)", example=12.5)
...
class CarteiraUpdatePremioDIRequest(BaseModel):
    ...
    premio: float = Field(..., description="Prêmio sobre DI", example=0.5)
    di: float = Field(..., description="Taxa DI de referência (%)", example=13.0)


class CarteiraUpdateDiasRequest(BaseModel):
    """Request model para atualizar dias de liquidação"""

    dias: int = Field(..., description="Novo número de dias para liquidação", ge=0, example=1)
...
class CarteiraUpdateQuantidadeRequest(BaseModel):
    ...
    quantidade: float = Field(..., description="Nova quantidade", gt=0, example=50000)
...
class CarteiraResponse(BaseModel):
    ...
    carteira_id: Optional[str] = Field(
        None, description="ID da carteira (retornado apenas na criação)"
    )
