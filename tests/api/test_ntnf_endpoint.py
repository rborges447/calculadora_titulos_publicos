"""Regressao POST /titulos/ntnf vs golden (Spec 001, Fase 5)."""
...
def test_post_ntnf_bate_golden(client):
    golden = load_golden("api_ntnf_response")
    resp = client.post("/titulos/ntnf", json=golden["request"])
    assert resp.status_code == 200
    body = resp.json()
    assert_response_vs_golden(body, golden["response"])

    core = load_golden("ntnf_2029_taxa_12_5")
